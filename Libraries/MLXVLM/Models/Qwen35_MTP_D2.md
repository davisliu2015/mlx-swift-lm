...
### 7.3 量化算法对照实验（MXFP4 vs 4bit affine，唯一变量=量化）

为隔离"量化算法"这一单一变量，用**同一份 Qwen3.6-27B、同一架构、同走 LLM 文本路径、
同一段 prompt、同样 D1/D2 配置**的两个发布版本对照（此前 VLM/LLM 对比因走不同代码路径、
不同模型，无法归因）：

| | MXFP4 `Qwen3.6-27B-MXFP4-VL-MTP` | 4bit affine `samwang0041/Qwen3.6-27B-MLX-4bit-MTP` |
|---|---|---|
| 主干量化 | `mode=mxfp4, bits=4, group_size=32` | `mode=affine, bits=4, group_size=64` |
| MTP 头 | `mtp_preserved`（高精度保留） | 普通 4bit 量化 |
| 磁盘权重总大小 | **15 GB** | 14 GB |
| Baseline（无 MTP） | **8.64 / 8.65** | 8.22 / 8.15 |
| D1（MTP 1-draft） | 13.30 / 13.52 | 13.68 / 13.73 |
| D2-快照 | 14.62 / 14.42 | **15.71 / 15.71** |
| **d1 命中率** | **84%** | **88%** |
| d2 命中率 | 44% | 54% |
| D2 相对 Baseline | +67~69% | **+91~93%** |
| C vs D 自检误差 | 0.00e+00 ✅ | 0.00e+00 ✅ |

（速度单位 tok/s，temp=0 两轮；temp=0.7 因采样随机性 + C/D 生成路径分叉波动大，不作结论依据。）

**结论一：投机解码的加速由命中率主导，不是单步 forward 速度。**
4bit affine 的 d1/d2 命中率更高（88% vs 84%、54% vs 44%），因此 D2 绝对速度反超 MXFP4
（15.7 vs 14.5）。这直接推翻了"高精度 MTP 头命中更准"和"量化头预测更快带来加速"两种猜测：
**MTP 头的量化方式与最终命中率之间没有可预测的正相关**，命中率更取决于该权重/训练本身。

**结论二：MXFP4 的 Baseline 反而略快（~6%），但主因不是"搬的权重更少"。**
实测 MXFP4 磁盘权重更大（15 GB > 14 GB），decode 是访存瓶颈却仍更快，说明差异来自
**反量化 kernel 效率**：`mxfp4` 是共享指数浮点格式（查 4bit 尾数表 + 应用 block 指数、
无 per-group bias），反量化算子比 `affine` 的"乘 scale + 加 bias"更省；这点收益在 Baseline
逐 token 场景被放大成 ~6%。属次要因素，且量级在正常波动范围内，不宜过度解读。

**结论三：D2 相对提升 % 会被较慢的 Baseline 放大**，看绝对速度更公允：
两模型 D2 都在 14.5~15.7 tok/s 区间，差距其实不大。
...
6. **量化算法（第 7.3 节对照）**：加速幅度由**命中率**主导，单步 forward 速度是次要因素；
   MXFP4 与 4bit affine 谁的命中率更高**无法从量化算法预测**（本例 affine 反而更高）。
   选量化时应以目标模型的实测命中率为准，而非"精度越高命中越准"的直觉。
